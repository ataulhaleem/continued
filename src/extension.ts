...
export function activate(context: vscode.ExtensionContext) {
    const provider = new ContinuedSidebarProvider(context.extensionUri, context.globalState);
...
class ContinuedSidebarProvider implements vscode.WebviewViewProvider {
    ...
    private _getAllSessions(): ChatSession[] {
        return this._globalState.get<ChatSession[]>('continued_sessions', []);
    }

    private _saveSessions(sessions: ChatSession[]) {
        this._globalState.update('continued_sessions', sessions);
    }
            ...
            switch (data.type) {
                case 'getModels': {
                    try {
                        ...
                        webviewView.webview.postMessage({ type: 'setModels', models: ['llama3'] });
                    }
                    ...
                    if (this._lastFileSnapshot) {
                        ...
                        if (workspaceFolders) {
                            ...
                            if (data.action === 'Keep') {
                                ...
                                if (this._lastFileSnapshot.isDeletion) {
                                    // Resurrection: Write the file back to disk from memory snapshot
                                    try {
...
let dynamicPayload: ChatMessage[] = [
                    ...
                    try {
                        ...
                        if (mode === 'agent' || mode === 'agent-auto') {
                            // 1. Process Write/Edit Request Match Loop
                            const writeMatch = completeResponse.match(/<write_file\s+path=["']([^"']+)["']\s*>([\s\S]*?)<\/write_file>/);
                            // 2. Process Deletion Request Match Loop
    ...
    private async _executeFileDeletion(relativePath: string) {
        ...
        try {
            ...
            // File already non-existent, exit early
            return;
        }

        // Cache the content snapshot so Discard acts as a complete undelete action
                ...
                window.addEventListener('message', event => {
                    ...
                    switch (message.type) {
                        ...
                        case 'requestDeleteApproval':
                            deleteMsg.innerText = "🚨 CRITICAL: Agent wants to DELETE '" + message.filename + "'. Allow?";
                            deletePanel.style.display = 'flex';
                            break;

                        case 'showSaveDiscardPanel':
                            if (message.msgType === 'delete') {
                                saveDiscardMsg.innerText = "🗑️ File '" + message.filename + "' was deleted. Keep changes or Undo?";
                            } else {
                                saveDiscardMsg.innerText = "💾 Edits applied to '" + message.filename + "'. Keep or Discard?";
                            }
