...
2. **Submit a New Add-on** → *On this site* (listed) → upload `continued-firefox-<version>.zip`. The validator runs immediately. Expected result: 0 errors, 0 warnings (the build rewrites the UI's `innerHTML` assignments to a DOMParser helper, which is what the linter's *Unsafe assignment to innerHTML* check wants). Errors block; if a warning appears after a UI change, check `rewriteInnerHtml` in `build.mjs`.
   - The manifest declares `data_collection_permissions: { required: ["websiteContent", "websiteActivity"] }`. Firefox 140+ shows this to users as a consent prompt at install. It is accurate: page text, tab titles and URLs are sent to the AI provider the user selected, and to nobody else. Do not change it to `none`; that would be a false declaration.
   - **Notes to reviewer** (paste into the "Notes for Reviewers" box): *Markup in the side panel is generated by a small Markdown renderer from HTML-escaped text and inserted via DOMParser (`__setHTML` in shim.js); no user-controlled markup reaches the DOM unescaped and the CSP is `script-src 'self'`. The extension transmits data only to the AI provider the user configures in the options page (Ollama on the user's own machine by default); there is no developer backend. Source is in the `browser-plugin/` folder of the linked repository; build with `cd browser-plugin && npm ci && npm run build` (output `dist/firefox`).*
